...
if __name__ == "__main__":
    ...
    if create_plots:
        if pos_neg_ece:
            ...
            plt.ylabel('positive ECE', fontsize=18)
            ...
            plt.xlabel('accuracy', fontsize=18)
            plt.ylabel('negative ECE', fontsize=18)
            ...
            plt.figure()
            plt.scatter(p_acc, (p_csece_pos + p_csece_neg).cpu())
            #plt.title('Classes neg ECE vs. classes accuracy before scaling, {}, {}'.format(dataset, args.model))
            plt.xlabel('accuracy', fontsize=18)
            plt.ylabel('combined ECE', fontsize=18)
            plt.savefig(os.path.join(save_plots_loc, '{}_{}'.format(dataset, args.model), 'combined_ece_acc_before_scaling_{}_{}.eps'.format(dataset, args.model)), format='eps', dpi=40)
            plt.close()
